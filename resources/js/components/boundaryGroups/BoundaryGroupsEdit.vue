<template>
    <div style="min-height: 80vh">
        <div class="row">
            <div class="col-12 col-md-6 pr-0">
                ...
                <div class="row w-100 mx-auto my-2 align-items-center justify-content-center">
                    <div class="col-12 pl-0 pr-2">
                        ...
                        <textarea style="border-color: #dfe1ee; background-color: transparent; border-radius: 3px; height: 152px"
                                  class="w-100"
                                  v-model="edited_group.description"
                        ></textarea>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
                        <div class="row" style="align-items: baseline; justify-content: flex-end">
            ...
            <div class="col-md-3 pr-0">
                ...
                <div class="row my-1">
                    <div class="col-8 pr-0">
                        <input placeholder="Поиск..." class="form-control custom-input" type="text" v-model="all_boundary_filter">
                    </div>
                    <div class="col-4 pl-1">
                        <span v-on:click="selectAll()" style="font-size: 10px; border-bottom: 1px solid #7224f2; cursor: pointer">Выбрать все</span>
                    </div>
                </div>
                <ul class="list-group list-scrollable">
                    <li  v-on:click="select(device)" v-for="device in allBoundaryFilter" class="list-group-item">
            ...
            <div class="col-md-3 pr-0">
                <div>Выбранные рубежи</div>
                <div class="row my-1">
                    <div class="col-8 pr-0">
                        <input placeholder="Поиск..." class="form-control custom-input" type="text" v-model="select_boundary_filter">
                    </div>
                    <div class="col-4 pl-1">
                ...
                <ul class="list-group list-scrollable">
                    <li v-on:click="deselect(device)" v-for="device in selectedBoundaryFilter" class="list-group-item">
                        <div style="display: flex;align-items: center">
                            <svg style="height: 14px;width: 14px;margin-right: 5px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                 viewBox="0 0 487.65 487.65" xml:space="preserve">
                            <g>
...
export default {
    name: "AppealsAdd",
    props: ['boundaries', 'group', 'connectedboundaries'],
    created() {
        this.selected = this.connectedboundaries;
        this.boundaries.forEach((boundary) => {
            let removeIndex = this.selected.map(item => item.id).indexOf(boundary.id);
            if (removeIndex < 0) {
                this.not_selected.push(boundary)
            }
        });
        //this.not_selected = this.devices;
        this.edited_group.title = this.group.title;
        this.edited_group.description = this.group.description;
    },
    data: () => ({
        all_boundary_filter:'',
        select_boundary_filter:'',
        selected: [],
        not_selected: [],
        edited_group: {},
        errors: {
    ...
    computed: {
        allBoundaryFilter: function () {

            return this.all_boundary_filter === '' ? this.not_selected :
                this.not_selected
                    .filter(item => item.title.toLowerCase().indexOf(this.all_boundary_filter.toLowerCase()) !== -1)
        },

        selectedBoundaryFilter: function () {


            return this.select_boundary_filter === '' ? this.selected :
                this.selected
                    .filter(item => item.title.toLowerCase().indexOf(this.select_boundary_filter.toLowerCase()) !== -1)
        }
    },
    methods: {
        selectAll() {
        ...
        select(device) {
            let removeIndex = this.not_selected.map(item => item.id).indexOf(device.id);
        ...
        submit() {
            this.loading = true
            this.edited_group.boundaries = this.selected;
            axios
                .patch(`/boundary-group/${this.group.id}`, this.edited_group)
                .then(() => {
