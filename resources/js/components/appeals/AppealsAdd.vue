            ...
            <div class="col-12 col-md-6 pr-0">
                ...
                >
                    ...
                    <div class="col-6 pl-0 pr-2">
                        ...
                        <select
                            v-model="new_appeal.responsible_id"
                            class="form-select custom-select"
                ...
                >
                    <div class="col-6 pl-0 pr-2">
                        ...
                        <select
                            v-model="new_appeal.malfunction_id"
                            class="form-select custom-select"
                    ...
                    <div class="col-6 pl-0 pr-2">
                        ...
                        <input
                            :disabled="new_appeal.malfunction_id === 29"
                            v-model="new_appeal.custom_reason"
...
export default {
    name: "AppealsAdd",
    props: ["users", "boundaries", "devices", "malfunctions", "user"],
    created() {
        if (localStorage.getItem("appeal")) {
            let appeal = JSON.parse(localStorage.getItem("appeal"));

            localStorage.removeItem("appeal");

            this.new_appeal.boundary_id = appeal.device.device_boundary_id;

            this.new_appeal.device_id = appeal.device.id;

            let boundary = this.boundaries.filter(
                item => item.id === appeal.device.device_boundary_id
            );
            ...
            setTimeout(() => {
                this.selected_device = {
                    label: appeal.device.serial,
                    code: appeal.device.id
                };
            }, 1000);
        }
    },
    ...
    computed: {
        devicesOptions() {
            ...
            if (!this.new_appeal.boundary_id) {
                tmp = this.devices;
            } else {
    ...
    methods: {
        ...
        submit() {
            this.loading = true;
            this.new_appeal.user_id = this.user.id;
            this.new_appeal.notify = this.notify_by_mail;
            axios
                .post("/task", this.new_appeal)
                .then(resp => {
                    this.sendNotify("Задача успешно создана", "success");
                    setTimeout(() => {
