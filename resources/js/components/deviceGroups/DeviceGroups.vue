    ...
    <div>
        ...
        >
            <div
                ...
                class="custom-filter col-8 pl-0"
            >
                Название группы<i
                    v-bind:class="{
                ...
                >
                    <div class="col-12 col-md-8">
                        ...
                        >
                            ...
                            <a  :disabled="!can('Просмотр группы устройств')"
                                class="serial-number"
                                :href="`/device-groups/${item.id}`"
                            >
                                {{ item.title }}
                            </a>
                        </div>
                    ...
                    <div class="col-12 col-md-2">
                        ...
                        >
                            ...
                            <a :disabled="!can('Восстановление группы устройств')" href="#restore" style="color:green;" @click="restore(item)">Восстановить</a>
                        </div>
                        <div
                            v-if="!item.deleted_at"
                        ...
                        >
                            <div>
                                <!--                                    v-if="can('Изменение группы устройств')"-->
                                <button

                                    class="btn p-1 mr-2 mdi mdi-pencil"
                                    v-on:click="editDevice(item.id)"
                                    style="height:36px; border: 1px solid #dfe1ee; font-size: 15px; width: 36px; min-width: 36px; background: transparent; color: black"
                                ></button>
                            </div>
<!--   :disabled="!can('Удаление группы устройств')" -->
                            <button
                ...
                <div class="row d-flex justify-content-center">
                    <div class="col-4">
                        <button class="btn w-100 btn-dark my-1 mr-1 py-2"
                                :disabled="!can('Удаление группы устройств')"
                                @click="deleteDeviceGroup( removedItem.id)"
...
export default {
    ...
    methods: {
        ...
        restore(item) {
            ...
            axios
                .get(`/device-groups/restore/${item.id}`)
                .then(resp => {
                    this.sendNotify('Вы успешно восстановили группы устройств', 'success');
                    window.location.href = `/device-groups`;
                })
                .catch(() => {
                    this.sendNotify('Ошибка восстановления группы устройств', 'error');
                });
        ...
        deleteDeviceGroup(id) {
            ...
            axios
                ...
                .then(resp => {
                    console.log(resp);
                    this.sendNotify('Вы успешно удалили группу устройств', 'success');
                    $('#accept-remove').modal('hide')
                })
                .catch(()=> {
                    $('#accept-remove').modal('hide')
                    this.sendNotify('Ошибка удаления группы устройств', 'error');
                });
        },
