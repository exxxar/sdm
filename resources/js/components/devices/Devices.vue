    ...
    <div>
        ...
        >
            <div class="col-8">
                <div class="row w-100 mx-auto align-items-center">
                    ...
                    >
                        <div class="mr-1">{{ device.serial }}</div>
                        <div
        ...
        >
            <div
                ...
                class="custom-filter col-2 pl-0"
            >
                Серийный номер ({{ sortedList.length }})
                <i
                    v-bind:class="{
            ...
            >
                Группа<i
                    ...
                    v-if="currentSort == 'device_group_id'"
                    class="fas fa-angle-down filter-icon"
                ></i>
            </div>
            <div
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            {{ item.type ? item.type.title : "ошибка" }}
                        </div>
                    </div>
                    <div class="col-12 col-sm-6 col-md-2">
                        <div v-if="item.group" class="d-flex table-text">
                            <a
                                class="additional-link"
                                :href="`/device-groups/${item.group.id}`"
                            >
                                {{ item.group ? item.group.title : "ошибка" }}
                                <i
                    ...
                    <button
                        ...
                        title="Просмотреть изображение с камеры"
                    >
                        <svg
                            version="1.1"
                            id="Capa_1"
                        ...
                        >
                            ...
                            <div style="display: flex" class="mr-2">
                                ...
                                >
                                    <svg
                                        v-bind:class="{
                                            rotating: refreshingDevices.includes(
                                                item.id
                                            )
                                        }"
                                        version="1.1"
                                ...
                                <button
                                    ...
                                    :disabled="!can('Изменение устройств')"
                                    title="Редактировать"
                                >
                                    <svg
                                        id="Capa_1"
                                ...
                                <button
                                    v-if="!item.deleted_at"
                                    class="btn"
                                    :disabled="!can('Удаление устройств')"
        ...
        <modal id="accept-remove">
            ...
            <template v-slot:main>
                <h4 class="w-100 text-center" v-if="removedItem">
                    Вы хотите удалить ?
                </h4>
                ...
                <div class="row d-flex justify-content-center">
                    <div class="col-4">
                        <button
                            ...
                            :disabled="!can('Удаление устройств')"
                            @click="deleteDevice(removedItem.id, true)"
        ...
        <video-modal v-if="selectedDevice" id="video-from-device">
            <template v-slot:header>
                Видеопоток с камеры {{ selectedDevice.serial }}
            </template>
            <template v-slot:main>
                ...
                <iframe
                    v-if="selectedDevice.device_type_id == 2"
                    :src="'http://81.200.248.156:3001/?id=' + selectedDevice.id"
                    height="460px"
                    width="760px"
                ></iframe>
                <img
                    id="img"
                    v-if="
                        selectedDevice.device_type_id == 5 ||
                            selectedDevice.device_type_id == 3
                    "
...
export default {
            ...
            return this.data.filter(item => {
                ...
                if (this.groupId !== "") {
                    if (item.device_group_id) {
                        group = item.device_group_id === this.groupId;
                    } else {
                        group = false;
                    }
                }
    ...
    methods: {
        ...
        nextSlide() {
            this.cashableImgIndex++;
            this.currentImage = this.cashableImg[this.cashableImgIndex];

            if (this.cashableImg.length > this.cashableImgIndex)
                this.cashableImgIndex++;
        },
        can(permission) {
        ...
        openStream(item) {
            ...
            if (item.device_type_id == 5 || item.device_type_id == 3) {
                this.currentImage = `http://81.200.248.156:3000/video?login=${
                    this.selectedDevice.login
                }&start_time=${new Date().getTime()}&ip=${
                    this.selectedDevice.ip
                }&password=${
                    this.selectedDevice.password
                }&end_time=${new Date().getTime()}&device_type=${
                    this.selectedDevice.device_type_id
                }`;
            }
        },
        makeRequest() {
            //todo: если модалка открыта, то выполнять

            const axiosSource = axios.CancelToken.source();
            this.request = { cancel: axiosSource.cancel, msg: "Loading..." };
            //this.currentImage = "";
            axios
                .get(`/device/image/${this.selectedDevice.id}`, {
                    cancelToken: axiosSource.token
                })
                .then(resp => {
                    this.modalLoading = false;
                    /*this.currentImage =
                        resp.data + "?t=" + new Date().getTime();*/

                    this.cashableImg.push(
                        resp.data + "?t=" + new Date().getTime()
                    );

                    //console.log(this.cashableImg);
                    this.clearOldRequest("Success");
                    clearInterval(this.timer);
                    this.makeRequest();
                    //this.timer = setInterval(() => this.makeRequest(), 1000);
                })
                .catch(error => {
                    if (error.response) {
        ...
        restore(item) {
            axios
                .get(`/devices/restore/${item.id}`)
                .then(resp => {
                    this.sendNotify(
                        "Вы успешно восстановили устройство",
                        "success"
                    );
        ...
        deleteDevice(id) {
            ...
            axios
                ...
                .then(resp => {
                    //console.log(resp);
                    this.sendNotify("Вы успешно удалили устройство", "success");
        ...
        refreshDevice(device) {
            ...
            this.refreshingDevices.push(device.id);
            axios
                .post(`/devices/current-info/${device.id}`, {
                    ip: device.ip,
                ...
                .then(resp => {
                    const refreshIndex = this.refreshingDevices.indexOf(
                        device.id
                    );
                    this.refreshingDevices.splice(refreshIndex, 1);
                    const status = resp.data.status;
                    const ping = resp.data.ping ? resp.data.ping : 0;
                    this.data[index].ping = ping;

                    if (ping == 0) {
                        this.data[index].is_alive = false;
                    } else {
                        this.data[index].is_alive = true;
                    }
